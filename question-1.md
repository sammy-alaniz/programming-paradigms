...
- `\b` This asserts a word boundary, ensuring that `abstract` is recognized as a standalone word and not part of a larger word.
 ...
 #### b) identifier that has to start with `_` followed by any number of digits and lower case letters.
```
_[a-z0-9]*
```
Note: 
...
#### c) identifier that either (a) starts and ends with `_` and has any number of letters in between, or (b) contains only letters followed by digits only.
```
(_[a-zA-Z]+_)|([a-zA-Z]+\d+)
```
Note:
